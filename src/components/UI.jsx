...
// export default function UI({ assets, device, onThemeToggle }) {
//   const [isIntroPlaying, setIsIntroPlaying] = useState(false);

//   // All animation logic now lives cleanly in this UI component.
//   useEffect(() => {
...
//             .to(assets.nodes.Cube.scale, { x: 1.4, y: 1.4, z: 1.4, ease: "back.out(2.5)", duration: 0.7 })
//             .to(assets.room.position, { x: device === 'desktop' ? -1 : 0, z: device === 'desktop' ? 0 : -1, duration: 0.7, ease: "power1.out" }, "<")
...
//             .to(assets.room.position, { x: 0, y: 0, z: 0, duration: 1.0, ease: "power1.out" }, "sync")
//             .to(assets.nodes.Cube.rotation, { y: 2 * Math.PI + Math.PI / 4 }, "sync")
//             .to(assets.nodes.Cube.scale, { x: 5, y: 5, z: 5 }, "sync")
...
//             .to(assets.nodes.shelve?.scale, { x: 1, y: 1, z: 1, ease: "back.out(2.2)", duration: 0.5 }, "reveal+=0.2")
//             .to(assets.nodes.clock?.scale, { x: 1, y: 1, z: 1, ease: "back.out(2.2)", duration: 0.5 }, ">-0.4")
//             .to(assets.nodes.desk?.scale, { x: 1, y: 1, z: 1, ease: "back.out(2.2)", duration: 0.5 }, ">-0.4")
//             .to(assets.nodes.drawer?.scale, { x: 1, y: 1, z: 1, ease: "back.out(2.2)", duration: 0.5 }, "dr")
//             .to(assets.nodes.tableitem?.scale, { x: 1, y: 1, z: 1, ease: "back.out(2.2)", duration: 0.5 }, ">-0.4")
//             .to(assets.nodes.monitor?.scale, { x: 1, y: 1, z: 1, ease: "back.out(2.2)", duration: 0.5 }, ">-0.4")
//             .to(assets.nodes.chair?.scale, { x: 1, y: 1, z: 1, ease: "back.out(2.2)", duration: 0.5 }, "chair-anim")
//             .to(assets.nodes.chair?.rotation, { y: 4 * Math.PI, ease: "power2.out", duration: 1 }, "chair-anim");
//         };
